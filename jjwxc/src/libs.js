...
function getBookId(path) {
    var match = path.match(/\/read\/(\d+)/);
...
function formatJJWXCUrl(url) {
    if (url.includes('?')) {
        if (!url.includes('more=0&whole=1')) {
            return url.endsWith('&') ? url + 'more=0&whole=1' : url + '&more=0&whole=1';
        }
        return url;
    } else {
        return url.endsWith('/') ? url + '?more=0&whole=1' : url + '/?more=0&whole=1';
